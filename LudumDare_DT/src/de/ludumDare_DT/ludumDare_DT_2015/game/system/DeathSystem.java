...
public class DeathSystem extends IteratingSystem {
	...
	protected void processEntity(Entity entity, float deltaTime) {
		...
		if (deathComp.timer > 0) {
			...
			positionComp.y -= GameConstants.BOX2D_SCALE;
		} else {
			if (CompMappers.player.has(entity)) {
				EntityCreator.engine.removeAllEntities();
				MapLoader.generateWorldFromTiledMap(EntityCreator.engine,
						MapLoader.currentMap, EntityCreator.physicsSystem,
